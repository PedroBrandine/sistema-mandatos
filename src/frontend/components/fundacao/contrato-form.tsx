...
export function ContratoForm({ idContratante, contratosExistentes, modo, onConcluido }: ContratoFormProps) {
  ...
  const form = useForm<ContratoInput>({
    ...
    defaultValues:
      modo.tipo === "abrir"
        ? { id_produto: 0, dt_inicio: hoje(), status: "ativo" }
        : {
  ...
  async function enviar(valores: ContratoInput) {
    ...
    if (modo.tipo === "abrir") {
      const { error } = await supabase.from("fat_contrato").insert({
