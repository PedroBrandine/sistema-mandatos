-- Bug (Pedro, 24/09): "Vincular TSE" no PLL falhava com "Já existe um mandato
-- cadastrado com este título eleitoral" para todo parlamentar que já tinha
-- contrato em outro produto -- o PLL sempre pedia um dim_mandato novo e batia
-- em dim_mandato_nr_titulo_eleitoral_key.
--
-- O chamador (vincularParticipanteAoTse) agora resolve o mandato existente e
-- manda p_id_contratante_existente. Falta o lado do banco: nesse ramo,
-- app.criar_mandato ignorava p_candidatura, então o vínculo TSE confirmado
-- não era gravado e id_vinculo_tse voltava NULL (a ficha do participante
-- lê dados do TSE por esse id). Agora, com contratante existente +
-- p_candidatura, a função reaproveita a linha de rel_mandato_candidatura
-- daquela candidatura (confirmando-a se ainda não estava) ou cria uma nova.
-- Linhas antigas do mandato não são tocadas (histórico preservado).
--
-- O cadastro unificado (mandato-wizard) não é afetado: no fluxo "mandato
-- existente" ele manda p_candidatura NULL.
--
-- Mesma assinatura da versão anterior (20260922160558), só CREATE OR REPLACE.
