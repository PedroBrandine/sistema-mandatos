...
| Refresh+leitura: Server Component (`await atualizaNumerosImpacto(client); await buscarNumerosImpacto(client);` direto no `page.tsx`, `client` de `@backend/supabase/server`) em vez do `useState`/`useEffect` client-side de `IipCard` | Fetch server-side, mesmo padrão de `visao-gerencial/page.tsx`/blocos (`SaudeOperacaoBloco` etc., async Server Components) | `IipCard` é Client Component porque vive embutido dentro de uma árvore já client-side (`FichaContratoChrome`); `/numeros-impacto` é página nova, dedicada, sem essa restrição — Server Component é o padrão mais recente pra tela de Saída inteira (a irmã direta desta feature, `visao-gerencial-g3-g6`), evita round-trip extra client→PostgREST que um `useQuery` teria, e cada abertura de página já refaz o refresh naturalmente (Next.js App Router não cacheia Server Component por padrão). Mesmo comportamento observável (refresh-then-read a cada abertura), mecanismo mais simples. |
