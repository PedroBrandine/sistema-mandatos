...
- [ ] **D1.1** Liberar espaço em disco: Temp (8,1 GB), npm-cache (3,9 GB), `.next` (1,2 GB), pip cache (1,2 GB), Windows Update (0,6 GB) — meta: ~30 GB livres
- [ ] **D1.2** Decidir o destino da pasta `Dev Apps\Velhos` (3,1 GB, quase tudo `node_modules` de projetos antigos)
- [ ] **D1.3** Conferir a folga da máquina depois da limpeza (disco livre e RAM disponível)
- [ ] **D1.4** Travar a linha de base: `supabase migration list --linked` deve mostrar 24 linhas sincronizadas
- [ ] **D1.5** Rodar `supabase db advisors --linked` no dev e **guardar a saída** como gabarito para o D2.7
- [ ] **D1.6** Rodar `npm run lint`, `test:unit` e `test:integration` e registrar o que passa hoje
- [ ] **D1.7** Levantar tudo que **não é SQL** e não sobe com `db push`: auth hook, `site_url`, redirect URLs, SMTP, e SQL avulso rodado no Editor
- [ ] **D1.8** Guardar as chaves do `.env.local` de dev num gerenciador de senhas (serão usadas no Dia 3)
...
- [ ] **D2.1** Criar o projeto `sistema-mandatos-prod` no Supabase, região `sa-east-1`
- [ ] **D2.2** Guardar a senha do banco num gerenciador de senhas (não é recuperável)
- [ ] **D2.3** Rodar `supabase link --project-ref <ref-prod>` e `supabase db push` — as 24 migrations do zero
- [ ] **D2.4** Rodar `supabase config push` para ativar o `custom_access_token` hook
- [ ] **D2.5** Ajustar `site_url` e `additional_redirect_urls` para o domínio real (remover o IP de LAN)
- [ ] **D2.6** Configurar SMTP real, se o magic link for para usuários de verdade
- [ ] **D2.7** Rodar `supabase db advisors --linked` e comparar com o gabarito do D1.5
- [ ] **D2.8** Confirmar `supabase migration list --linked` com as 24 linhas local ↔ remote
- [ ] **D2.9** Testar o login ponta a ponta no prod — se logar e a tela vier vazia, o D2.4 falhou
- [ ] **D2.10** Voltar o link para o projeto de dev (`npnvoolkebhabjkjzqwn`)
...
- [ ] **D3.1** Cadastrar as 3 variáveis no ambiente **Production** com as chaves de prod
- [ ] **D3.2** Cadastrar as 3 variáveis em **Preview** e **Development** com as chaves de dev
- [ ] **D3.3** Conferir que `SUPABASE_SERVICE_ROLE_KEY` não tem prefixo `NEXT_PUBLIC_`
- [ ] **D3.4** Confirmar `master` como Production Branch em Settings → Git
- [ ] **D3.5** Criar e publicar a branch `develop`
